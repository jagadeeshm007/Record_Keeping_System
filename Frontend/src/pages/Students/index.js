...
const Dashboard = () =>{
  const [isModalOpen, setIsModalOpen] = useState(false);
    ...
    const fetchTableData = async () => {
        try {
            axiosAPI.get("http://localhost:5000/api/pending")
            .then((res)=>{
                if(res.status === 200){
    ...
    const handleGrantPermission = async (rowData) => {
      ...
      console.log(Roll)
        try {
           ...
           .then((res)=>{
            if(res.status === 200){
              const updatedTableData = tableData.filter(item => item.Roll !== Roll);
    ...
    const handleDenyPermission = async (rowData) => {
      let Roll = rowData.Roll
        try {
                            ...
                            <tbody>
                                {tableData.length && tableData.map((rowData, index) => (
                                    <tr key={index}>
                                        ...
                                        {/* <td><span className={`badge badge-soft-${rowData.Status === "Pending" ? "success" : rowData.Status === "Denied" ? "danger" : "warning"} font-size-12`}>{rowData.Status}</span></td> */}
